 ...
 *
 * The variants are the opposite: each one is a real ClinVar record, and its
 * historical classification is exactly what ClinVar itself said about that
 * variant in its January 2023 public release (NCBI archive
 * variant_summary_2023-01). The classification, review status and dbSNP
 * identifier of all twelve were re-checked against the GRCh38 rows of that
 * archive on 2026-09-25. Every "drift" in this workspace is therefore a real,
 * dated reclassification that anyone can verify; the release-by-release history
...
/** A dated public release that a historical classification was read from. */
...
export interface MonitoredVariant {
  ...
  rsid: string | null;
  /** The interpretation on record: ClinVar's classification in `historicalSource`. */
  historicalClassification: ClassificationCode;
  /** The same classification exactly as ClinVar worded it in that release. */
  historicalClinvarText: string;
  /** ClinVar review status attached to the classification in that release. */
  historicalReviewStatus: string;
